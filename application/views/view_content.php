...
<body>
    ...
    <?php
        ...
        $query = $this->db->query("SELECT * FROM tb_komentar WHERE komentar_status='0' AND komentar_content_id = '$content_id'");
        foreach ($query->result() as $utama):
    ?>
    <div class="container">
        <div class="w3-panel w3-pale-blue w3-leftbar w3-border-blue">
            <p>
                <b><?php echo $utama->komentar_nama?></b>
                <br><?php echo $utama->komentar_isi?>
                <br><button class="w3-button w3-tiny w3-blue" onclick="document.getElementById('<?php echo $utama->komentar_id?>').style.display='block'">Balas</button>
            </p>
        </div>
    </div>
        <?php
            ...
            $query = $this->db->query("SELECT * FROM tb_komentar WHERE komentar_status='$komentar_id' AND komentar_content_id = '$content_id'");
            foreach ($query->result() as $balasan):
        ?>
        <div class="container">
            <div class="w3-panel w3-pale-blue w3-leftbar w3-border-blue child">
                <p>
                    <b><?php echo $balasan->komentar_nama?></b>
                    <br><?php echo $balasan->komentar_isi?>
                </p>
            </div>
        </div>
        <?php endforeach;?>
            ...
            <div class="w3-modal-content modal">
                ...
                <div class="w3-container">
                    <form class="w3-container" method="POST" action="<?php echo site_url('komentar/balas') ?>">
                        <input type="hidden" name="komentar_id" value="<?php echo $utama->komentar_id?>">
