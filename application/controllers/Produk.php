...
class Produk extends CI_Controller
{
    ...
    function status2($id)
    {
        if ($this->produk_model->update_status2($id)) {
            redirect(site_url('produk/'));
        }
    }
    ...
    {
        if ($this->produk_model->update_banner($id)) {
            redirect(site_url('produk/'));
        }
    }

    function status_banner2($id)
    {
        if ($this->produk_model->update_banner2($id)) {
            redirect(site_url('produk/'));
        }
    }

    function status_produkbaru($id)
    {
        if ($this->produk_model->update_produkbaru($id)) {
            redirect(site_url('produk/'));
        }
    }

    function status_produkbaru2($id)
    {
        if ($this->produk_model->update_produkbaru2($id)) {
            redirect(site_url('produk/'));
        }
    }
